...
export default async function QuizQuestionPage({
  ...
  const [params, quiz, dictionary] = await Promise.all([
    resolveSearchParams(searchParams),
    getQuizQuestionData(),
    getDictionary(),
  ]);
  if (!quiz) {
    redirect(buildFlashUrl("/quiz", "warning", dictionary.quiz.noActiveSession));
  }
  ...
  const promptLang =
    quiz.quizType === "to_spanish" ? quiz.word.language.language : dictionary.quiz.spanish;
      ...
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mb-6 flex flex-col gap-5">
            <div className="flex items-center justify-between gap-4">
              <div>
                <span className="eyebrow">{dictionary.quiz.configEyebrow}</span>
                <h1 className="mt-2 font-display text-2xl font-semibold text-neutral-900">
                  {dictionary.quiz.question} {quiz.stats.answered + 1}
                  <span className="text-neutral-400"> / {quiz.stats.totalAvailable}</span>
                </h1>
              </div>
              <form action={endQuizAction}>
                <SubmitButton className="outline-button px-4 py-2.5 text-sm" icon="fa-solid fa-flag-checkered" pendingLabel={dictionary.quiz.ending}>
                  {dictionary.quiz.end}
                </SubmitButton>
              </form>
            </div>
          ...
          <div className="flex items-center justify-between border-b border-neutral-200 bg-neutral-50 px-6 py-3 sm:px-10">
            <span className="eyebrow-muted">
              {quiz.quizType === "to_spanish"
                ? dictionary.quiz.translateTo(dictionary.quiz.spanish)
                : dictionary.quiz.translateTo(quiz.word.language.language)}
            </span>
          ...
          <div className="px-6 py-12 text-center sm:px-10">
            ...
            {quiz.word.explanation ? (
              <div className="mx-auto mt-8 max-w-xl rounded-xl rounded-l-sm border-l-[3px] border-accent bg-neutral-50 px-5 py-4 text-left">
                <div className="eyebrow-muted mb-1">{dictionary.quiz.hint}</div>
          ...
          <div className="border-t border-neutral-200 bg-neutral-50 px-6 py-8 sm:px-10">
            <form action={submitQuizAnswerAction} className="space-y-5">
              ...
              <input
                ...
                id="answer"
                name="answer"
                placeholder={dictionary.quiz.answerPlaceholder}
                required
              />

              <SubmitButton className="primary-button w-full py-4 text-base" icon="fa-solid fa-check" pendingLabel={dictionary.quiz.checking}>
                {dictionary.quiz.verify}
              </SubmitButton>
            </form>
            ...
            <div className="mt-3 flex flex-col gap-3 sm:flex-row">
              <form action={skipQuizAnswerAction} className="flex-1">
                <input name="wordId" type="hidden" value={quiz.word.id} />
                <input name="sessionId" type="hidden" value={quiz.session.sessionId} />
                <SubmitButton className="outline-button w-full" icon="fa-solid fa-forward" pendingLabel={dictionary.quiz.skipping}>
                  {dictionary.quiz.skip}
                </SubmitButton>
              </form>
              <Link className="outline-button flex-1" href="/verpalabras">
                <i className="fa-solid fa-book" />
                <span>{dictionary.quiz.viewLexicon}</span>
              </Link>
            </div>
          </div>
        </div>
